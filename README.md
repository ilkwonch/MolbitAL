...
MolbitAL is a modular active‑learning engine designed for ultra‑large virtual screening campaigns. It orchestrates end‑to‑end workflows—from ligand preparation and docking to machine‑learning‑guided acquisition—so you can triage **billions** of candidates efficiently on a single workstation or an HPC cluster. The framework plugs into popular docking back‑ends (AutoDock‑Vina, Vina‑GPU 2.1, QVina 2.1, QVina‑W, Uni‑Dock) and currently supports Random‑Forest (RF) and XGBoost‑Distribution (XGB‑D) surrogate models for fast affinity ranking.
...
* **Python ≥ 3.10**
* **Conda ≥ 23** (recommended)
* **CUDA 11+** (optional, for Vina‑GPU 2.1 and Uni‑Dock)
...
| `docking_engine` | `vina`, `vina-gpu`, `qvina`, `qvina-w`, or `unidock` |
| `ml_model` | `rf` or `xgbd` |
...
Below is the manual, step‑by‑step route (useful for debugging or partial integration).
